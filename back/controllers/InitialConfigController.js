...
const InitialCheck = async (req, res) => {
    try {
        ...
        if (!masterAdminRole) {
            return res.status(200).json(handleSuccessfulResponse('Initial setup required. No MasterAdmin role found.', {
                setupRequired: true,
        ...
        if (!masterAdminUser) {
            return res.status(200).json(handleSuccessfulResponse('Initial setup required. No MasterAdmin found.', {
                setupRequired: true,
                verificationRequired: true,
                masterAdminRequired: true
            }));
        ...
        if (masterAdminUser.verification !== 'active') {
            return res.status(200).json(handleSuccessfulResponse('MasterAdmin is not verified.', {
                setupRequired: true,
                verificationRequired: true,
        ...
        return res.status(200).json(handleSuccessfulResponse('System is fully configured and ready for use.', {
            setupRequired: false,
...
// const { ErrorHandler, handleErrorResponse } = require('../helpers/errorHandler');
// const { handleSuccessfulResponse } = require('../helpers/responseHelper');
// const logger = require('../helpers/logHelper');
...
//             }
//         }
//         res.status(200).json(handleSuccessfulResponse(message, { setupRequired, verificationRequired, masterAdminRequired }));
//     } catch (error) {
//         handleErrorResponse(error, req, res);
//     }
// };
